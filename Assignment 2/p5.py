...
def min_max_mulitple_ghosts(problem, k):
    ...
    while len(problem['pacLocation']) > 0 and len(problem['foodLocation']) > 0:
        if round == 0:
            ...
            direction = minimax(agentDict[character], agentDict[character], problem['layout'], 0, k, len(queue),tempFood, tempPac, tempGhost, 0, agentDict )
...
def minimax(initialAgent, agent, layout, depth, maxDepth, maxAgent, foodLocation, pacLocation, ghostLocation, score, agentDict):
    ...
    if agent == 0:
        ...
        for newLocation in val_list:
            ...
            maxValList.append(minimax(initialAgent, 1, layout, depth, maxDepth, maxAgent, tempFood, tempPac, tempGhost, tempScore, agentDict))
        ...
        if len(val_list) == 0:
            ...
            tempPac = copy.deepcopy(pacLocation)
            tempGhost = copy.deepcopy(ghostLocation)
            minValList.append(minimax(initialAgent, nextAgent, layout, tempDepth, maxDepth, maxAgent, tempFood, tempPac, tempGhost, score, agentDict))
        else:
            for newLocation in val_list:
                tempFood = copy.deepcopy(foodLocation)
                tempPac = copy.deepcopy(pacLocation)
                ...
                tempScore = moveAndGetActionScores(agentDict[agent], newLocation, tempFood, tempPac, tempGhost, score)
                minValList.append(
                    minimax(initialAgent, nextAgent, layout, tempDepth, maxDepth, maxAgent, tempFood, tempPac, tempGhost, tempScore, agentDict))

        minVal = min(minValList)
        if initialAgent == agent and depth == 0:
            indexList = [i for i, val in enumerate(minValList) if val == minVal]
            key_list = list(avaDirDict.keys())
            index = random.choice(tuple(indexList))
            return key_list[index]
        return minVal
...
def Move(character, layout, pacLocation, ghostLocation, foodLocation, score, choice):
    if choice == '':
        return choice, score
